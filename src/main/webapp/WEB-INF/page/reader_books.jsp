...
<div style="padding: 20px 550px 10px">
    <form method="post" action="reader_querybooks_do" class="form-inline" id="searchform">
        <div class="input-group">
            <input type="text" placeholder="输入关键字" class="form-control" id="search" name="searchWords"
                   class="form-control">
        ...
        <table class="table table-hover">
            ...
            <c:forEach items="${bookInfos}" var="book">
                <tr>
                    ...
                    <c:forEach var="lend" items="${noReturnBooksIds}">
                        <c:if test="${lend eq book.book_id}">
                            <c:set var="flag" value="true"/>
                        </c:if>
                    </c:forEach>
                    <c:if test="${flag}">
                        <td><a href="returnbook_do?bookId=<c:out value="${book.book_id}"></c:out>">
                            <button type="button" class="btn btn-danger btn-xs">归还</button>
                        </a></td>
                    </c:if>
                    <c:if test="${not flag}">
                        <c:if test="${book.number>0}">
                            <td><a href="lendbook_do?bookId=<c:out value="${book.book_id}"></c:out>">
                    ...
                    <td>
                        <a href="reader_book_detail.html?bookId=<c:out value="${book.book_id}"></c:out>">
